...
export default async function Home() {
  ...
  return (
    <MotionPage className="min-h-screen">
      <header className="glass-panel sticky top-0 z-20 mx-2 mt-2 rounded-2xl border-card-border/50">
      ...
      <main className="mx-auto w-full max-w-7xl px-4 pb-16 pt-8 sm:px-6">
        <section className="hero-background glass-panel rounded-3xl border border-card-border/70 px-6 py-16 text-center sm:px-10">
          <p className="pulse-glow mx-auto inline-flex items-center rounded-full border border-purple-400/40 bg-purple-500/10 px-4 py-1 text-sm text-purple-100">
          ...
          <h1 className="mx-auto mt-6 max-w-5xl text-4xl font-extrabold leading-tight [font-family:var(--font-space-grotesk)] sm:text-6xl lg:text-7xl">
            The easiest way to write <span className="neon-text-gradient">captions that make your music move</span>
          </h1>
          ...
          <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
            <Link href="/signup" className="glass-button rounded-full px-6 py-3 text-sm font-semibold sm:text-base">
              Try Free — 10 Credits
            </Link>
        ...
        <section id="faq" className="glass-card mt-10 rounded-2xl p-6">
      ...
      <footer className="glass-panel mx-2 mb-2 rounded-2xl px-6 py-8">
...
function PricingCard({
  ...
  return (
    <article className={`glass-card rounded-2xl p-5 ${highlight ? "shimmer-border border-purple-400" : ""}`}>
...
function FeatureVisual({ title, desc }: { title: string; desc: string }) {
  return (
    <article className="glass-card feature-texture rounded-2xl p-5">
